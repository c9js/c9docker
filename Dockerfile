...
#│ Базовые пакеты │
#└────────────────┘
RUN apk add g++ make python curl bash bash-doc bash-completion openssh-client git py-pip python-dev docker --no-cache \

#┌─────────────────────────────────────────────────────────┐
#│ Так как запустить cloud9 можно только на версии v8.x.x, │
#│ нам необходимо иметь на борту сразу две версии          │
#└─────────────────────────────────────────────────────────┘
&& apk add nodejs=16.16.0-r0 npm=8.1.3-r0  -X http://dl-cdn.alpinelinux.org/alpine/v3.15/main/ --no-cache --allow-untrusted \
...
&& curl http://dl-cdn.alpinelinux.org/alpine/v3.15/main/x86_64/nodejs-16.16.0-r0.apk     -o /apks/node/last/nodejs.apk \
